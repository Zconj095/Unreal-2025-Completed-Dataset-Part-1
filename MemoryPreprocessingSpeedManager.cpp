...
double UMemoryPreprocessingSpeedManager::Sigmoid(double x)
{
    x = FMath::Clamp(x, -50.0, 50.0);
    return 1.0 / (1.0 + FMath::Exp(-x));
}

double UMemoryPreprocessingSpeedManager::SigmoidDerivFromActivation(double a)
{
    return a * (1.0 - a);
}

void UMemoryPreprocessingSpeedManager::RandomizeParams(double Scale)
{
...
void UMemoryPreprocessingSpeedManager::InitializeNetwork(int32 InInputSize, int32 InHiddenNeurons, int32 InOutputSize)
{
...
void UMemoryPreprocessingSpeedManager::AddPreprocessingData(const TArray<double>& Features, double PreprocessingTime)
{
    Entries.Add(FMemoryPreprocessingData(Features, PreprocessingTime));
}

void UMemoryPreprocessingSpeedManager::ForwardSingle(const double* InX, TArray<double>& Z1, TArray<double>& A1, TArray<double>& Z2, TArray<double>& A2) const
{
...
double UMemoryPreprocessingSpeedManager::TrainNetwork(int32 Epochs, double LearningRate)
{
    ...
    for (int32 n = 0; n < N; ++n)
    {
        const FMemoryPreprocessingData& E = Entries[n];
        for (int i = 0; i < InputSize && i < E.Features.Num(); ++i) X[n * InputSize + i] = E.Features[i];
        Y[n * OutputSize + 0] = E.PreprocessingTime;
    }
...
double UMemoryPreprocessingSpeedManager::PredictPreprocessingTime(const TArray<double>& Features) const
{
...
double UMemoryPreprocessingSpeedManager::CalculateAveragePreprocessingTime() const
{
    if (Entries.Num() == 0) return 0.0;
    double Sum = 0.0;
    for (const FMemoryPreprocessingData& E : Entries)
    {
        Sum += E.PreprocessingTime;
    }
