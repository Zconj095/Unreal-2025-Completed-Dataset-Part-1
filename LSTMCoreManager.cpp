...
double ULSTMMemoryManager::Sigmoid(double x)
{
    x = FMath::Clamp(x, -50.0, 50.0);
    return 1.0 / (1.0 + FMath::Exp(-x));
}

double ULSTMMemoryManager::SigmoidDerivFromActivation(double a)
{
    return a * (1.0 - a);
}

void ULSTMMemoryManager::RandomizeParams(double Scale)
{
...
void ULSTMMemoryManager::InitializeLSTM(int32 InInputSize, int32 InHiddenNeurons, int32 InOutputSize)
{
...
void ULSTMMemoryManager::AddMemorySequence(const TArray<double>& Input, const TArray<double>& Output)
{
    Sequences.Add(FLSTMSequence(Input, Output));
}

void ULSTMMemoryManager::ForwardSingle(const double* InX, TArray<double>& Z1, TArray<double>& A1, TArray<double>& Z2, TArray<double>& A2) const
{
...
double ULSTMMemoryManager::TrainLSTM(int32 Epochs, double LearningRate)
{
    if (Sequences.Num() == 0 || InputSize == 0 || HiddenSize == 0 || OutputSize == 0)
    {
        return 0.0;
    }

    const int32 N = Sequences.Num();
    TArray<double> X; X.SetNumZeroed(N * InputSize);
    TArray<double> Y; Y.SetNumZeroed(N * OutputSize);

    for (int32 n = 0; n < N; ++n)
    {
        const FLSTMSequence& S = Sequences[n];
        for (int i = 0; i < InputSize && i < S.Input.Num(); ++i)
        {
            X[n * InputSize + i] = S.Input[i];
        }
        for (int o = 0; o < OutputSize && o < S.Output.Num(); ++o)
        {
            Y[n * OutputSize + o] = S.Output[o];
        }
    }
    ...
    for (int32 epoch = 0; epoch < Epochs; ++epoch)
    {
        ...
        for (int32 n = 0; n < N; ++n)
        {
            ...
            // Output layer deltas for each output neuron
...
TArray<double> ULSTMMemoryManager::PredictSequence(const TArray<double>& Input) const
{
    TArray<double> Result; Result.Init(0.0, OutputSize);
