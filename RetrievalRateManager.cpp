...
double URetrievalRateManager::Sigmoid(double x)
{
    x = FMath::Clamp(x, -50.0, 50.0);
    return 1.0 / (1.0 + FMath::Exp(-x));
}

double URetrievalRateManager::SigmoidDerivFromActivation(double a)
{
    return a * (1.0 - a);
}

void URetrievalRateManager::RandomizeParams(double Scale)
{
...
void URetrievalRateManager::InitializeNeuralNetwork(int32 InInputSize, int32 InHiddenNeurons, int32 InOutputSize)
{
...
void URetrievalRateManager::AddRetrievalEntry(const TArray<double>& Features, bool bIsRetrieved, float RetrievalTime)
{
    Entries.Add(FRetrievalEntry(Features, bIsRetrieved, RetrievalTime));
}

double URetrievalRateManager::CalculateRetrievalRate() const
{
    if (Entries.Num() == 0) return 0.0;
    int32 RetrievedCount = 0;
    for (const FRetrievalEntry& E : Entries)
    {
        if (E.bIsRetrieved) ++RetrievedCount;
    }
    return static_cast<double>(RetrievedCount) / static_cast<double>(Entries.Num());
}

double URetrievalRateManager::CalculateAverageRetrievalTime() const
{
    if (Entries.Num() == 0) return 0.0;
    double Sum = 0.0;
    for (const FRetrievalEntry& E : Entries)
    {
        Sum += static_cast<double>(E.RetrievalTime);
    }
    return Sum / static_cast<double>(Entries.Num());
}

void URetrievalRateManager::ForwardSingle(const double* InX, TArray<double>& Z1, TArray<double>& A1, TArray<double>& Z2, TArray<double>& A2) const
{
...
void URetrievalRateManager::TrainNetwork(int32 Epochs, double LearningRate)
{
    ...
    for (int32 n = 0; n < N; ++n)
    {
        const FRetrievalEntry& D = Entries[n];
        for (int i = 0; i < InputSize; ++i)
        {
            X[n * InputSize + i] = (i < D.Features.Num()) ? D.Features[i] : 0.0;
        }
        Y[n * OutputSize + 0] = D.bIsRetrieved ? 1.0 : 0.0;
    }
...
float URetrievalRateManager::PredictRetrievalLikelihood(const TArray<double>& Features) const
{
    ...
    return (OutputSize >= 1) ? static_cast<float>(A2[0]) : 0.f;
}
