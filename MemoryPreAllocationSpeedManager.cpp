...
double UMemoryPreAllocationSpeedManager::Sigmoid(double x)
{
    x = FMath::Clamp(x, -50.0, 50.0);
    return 1.0 / (1.0 + FMath::Exp(-x));
}

double UMemoryPreAllocationSpeedManager::SigmoidDerivFromActivation(double a)
{
    return a * (1.0 - a);
}

void UMemoryPreAllocationSpeedManager::RandomizeParams(double Scale)
{
...
void UMemoryPreAllocationSpeedManager::InitializeNetwork(int32 InInputSize, int32 InHiddenNeurons, int32 InOutputSize)
{
...
void UMemoryPreAllocationSpeedManager::AddPreAllocationData(const TArray<double>& Features, double PreAllocationTime)
{
    Entries.Add(FPreAllocationData(Features, PreAllocationTime));
}

void UMemoryPreAllocationSpeedManager::ForwardSingle(const double* InX, TArray<double>& Z1, TArray<double>& A1, TArray<double>& Z2, TArray<double>& A2) const
{
...
double UMemoryPreAllocationSpeedManager::TrainNetwork(int32 Epochs, double LearningRate)
{
    ...
    for (int32 n = 0; n < N; ++n)
    {
        const FPreAllocationData& E = Entries[n];
        for (int i = 0; i < InputSize && i < E.Features.Num(); ++i) X[n * InputSize + i] = E.Features[i];
        Y[n * OutputSize + 0] = E.PreAllocationTime;
    }
...
double UMemoryPreAllocationSpeedManager::PredictPreAllocationTime(const TArray<double>& Features) const
{
...
double UMemoryPreAllocationSpeedManager::CalculateAveragePreAllocationTime() const
{
    if (Entries.Num() == 0) return 0.0;
    double Sum = 0.0;
    for (const FPreAllocationData& E : Entries)
    {
        Sum += E.PreAllocationTime;
    }
