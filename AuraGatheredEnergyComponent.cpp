#include "AuraGatheredEnergyComponent.h"

#include "Components/SceneComponent.h"
#include "Particles/ParticleSystemComponent.h"
#include "NiagaraComponent.h"

UAuraGatheredEnergyComponent::UAuraGatheredEnergyComponent()
{
    PrimaryComponentTick.bCanEverTick = false;
}

void UAuraGatheredEnergyComponent::BeginPlay()
{
    Super::BeginPlay();
    ApplyContentAura();
}

TArray<USceneComponent*> UAuraGatheredEnergyComponent::GetAllLayers() const
{
...
void UAuraGatheredEnergyComponent::ApplyContentAura()
{
    for (USceneComponent* Layer : GetAllLayers())
    {
        ApplyGatheredEnergyToAura(Layer);
    }
}

void UAuraGatheredEnergyComponent::ApplyGatheredEnergyToAura(USceneComponent* AuraLayer)
{
    if (!AuraLayer)
    {
        return;
...
void UAuraGatheredEnergyComponent::ApplyToNiagara(UNiagaraComponent* NiagaraComp) const
{
    if (!NiagaraComp)
    {
        return;
...
void UAuraGatheredEnergyComponent::ApplyToCascade(UParticleSystemComponent* ParticleComp) const
{
    if (!ParticleComp)
    {
        return;
