...
double URecollectionSpeedManager::Sigmoid(double x)
{
    x = FMath::Clamp(x, -50.0, 50.0);
    return 1.0 / (1.0 + FMath::Exp(-x));
}

double URecollectionSpeedManager::SigmoidDerivFromActivation(double a)
{
    return a * (1.0 - a);
}

void URecollectionSpeedManager::RandomizeParams(double Scale)
{
...
void URecollectionSpeedManager::InitializeNeuralNetwork(int32 InInputSize, int32 InHiddenNeurons, int32 InOutputSize)
{
...
void URecollectionSpeedManager::AddRecollectionEntry(const TArray<double>& Features, float RecallTime, float RetentionTime)
{
    Entries.Add(FRecollectionEntry(Features, RecallTime, RetentionTime));
}

double URecollectionSpeedManager::CalculateAverageRecollectionSpeed() const
{
    if (Entries.Num() == 0) return 0.0;
    double Sum = 0.0;
    for (const FRecollectionEntry& E : Entries)
    {
        Sum += static_cast<double>(E.RecollectionSpeed);
    }
    return Sum / static_cast<double>(Entries.Num());
}

void URecollectionSpeedManager::ForwardSingle(const double* InX, TArray<double>& Z1, TArray<double>& A1, TArray<double>& Z2, TArray<double>& A2) const
{
...
void URecollectionSpeedManager::TrainNetwork(int32 Epochs, double LearningRate)
{
    ...
    for (int32 n = 0; n < N; ++n)
    {
        const FRecollectionEntry& D = Entries[n];
        for (int i = 0; i < InputSize; ++i)
        {
            X[n * InputSize + i] = (i < D.Features.Num()) ? D.Features[i] : 0.0;
        }
        Y[n * OutputSize + 0] = static_cast<double>(D.RecollectionSpeed);
    }
...
float URecollectionSpeedManager::PredictRecollectionSpeed(const TArray<double>& Features) const
{
