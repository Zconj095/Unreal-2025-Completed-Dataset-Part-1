#include "AuraNaturalEnergyComponent.h"

#include "Components/SceneComponent.h"
#include "Particles/ParticleSystemComponent.h"
#include "NiagaraComponent.h"

UAuraNaturalEnergyComponent::UAuraNaturalEnergyComponent()
{
    PrimaryComponentTick.bCanEverTick = false;
}

void UAuraNaturalEnergyComponent::BeginPlay()
{
    Super::BeginPlay();
    ApplyContentAura();
}

TArray<USceneComponent*> UAuraNaturalEnergyComponent::GetAllLayers() const
{
...
void UAuraNaturalEnergyComponent::ApplyContentAura()
{
    for (USceneComponent* Layer : GetAllLayers())
    {
        ApplyNaturalEnergyToAura(Layer);
    }
}

void UAuraNaturalEnergyComponent::ApplyNaturalEnergyToAura(USceneComponent* AuraLayer)
{
    if (!AuraLayer)
    {
        return;
...
void UAuraNaturalEnergyComponent::ApplyToNiagara(UNiagaraComponent* NiagaraComp) const
{
    if (!NiagaraComp)
    {
        return;
...
void UAuraNaturalEnergyComponent::ApplyToCascade(UParticleSystemComponent* ParticleComp) const
{
    if (!ParticleComp)
    {
        return;
