...
double UNanoMeasurementSpeedTimersManager::Sigmoid(double x)
{
    x = FMath::Clamp(x, -50.0, 50.0);
    return 1.0 / (1.0 + FMath::Exp(-x));
}

double UNanoMeasurementSpeedTimersManager::SigmoidDerivFromActivation(double a)
{
    return a * (1.0 - a);
}

void UNanoMeasurementSpeedTimersManager::RandomizeParams(double Scale)
{
...
void UNanoMeasurementSpeedTimersManager::InitializeNetwork(int32 InInputSize, int32 InHiddenNeurons, int32 InOutputSize)
{
    InputSize = InInputSize;
    HiddenSize = InHiddenNeurons;
    OutputSize = InOutputSize; // typically 1
    RandomizeParams(0.1);
}

void UNanoMeasurementSpeedTimersManager::AddTimerData(const TArray<double>& Features, double NanoSeconds)
{
    TimerEntries.Add(FNanoTimerData(Features, NanoSeconds));
}

void UNanoMeasurementSpeedTimersManager::ForwardSingle(const double* InX, TArray<double>& Z1, TArray<double>& A1, TArray<double>& Z2, TArray<double>& A2) const
{
...
double UNanoMeasurementSpeedTimersManager::TrainNetwork(int32 Epochs, double LearningRate)
{
    ...
    for (int32 n = 0; n < N; ++n)
    {
        const FNanoTimerData& D = TimerEntries[n];
        for (int i = 0; i < InputSize; ++i)
        {
            X[n * InputSize + i] = (i < D.Features.Num()) ? D.Features[i] : 0.0;
        }
        Y[n * OutputSize + 0] = D.NanoSeconds;
    }
...
double UNanoMeasurementSpeedTimersManager::PredictNanoSeconds(const TArray<double>& Features) const
{
...
double UNanoMeasurementSpeedTimersManager::CalculateAverageTime() const
{
    if (TimerEntries.Num() == 0) return 0.0;
    double Sum = 0.0;
    for (const FNanoTimerData& E : TimerEntries)
    {
        Sum += E.NanoSeconds;
    }
