...
double UAttentionSpanManager::Sigmoid(double x)
{
    x = FMath::Clamp(x, -50.0, 50.0);
    return 1.0 / (1.0 + FMath::Exp(-x));
}

double UAttentionSpanManager::SigmoidDerivFromActivation(double a)
{
    return a * (1.0 - a);
}

void UAttentionSpanManager::RandomizeParams(double Scale)
{
...
void UAttentionSpanManager::InitializeNetwork(int32 InInputSize, int32 InHiddenNeurons, int32 InOutputSize)
{
...
void UAttentionSpanManager::AddAttentionData(double TFocus, double D, double Lambda)
{
    const double AS = CalculateAttentionSpan(TFocus, D, Lambda);
    AttentionDataEntries.Add(FAttentionData(TFocus, D, Lambda, AS));
}

double UAttentionSpanManager::CalculateAttentionSpan(double TFocus, double D, double Lambda) const
{
    // TFocus / (1 + Lambda * D)
    return TFocus / (1.0 + Lambda * D);
}

void UAttentionSpanManager::ForwardSingle(const double* InX, TArray<double>& Z1, TArray<double>& A1, TArray<double>& Z2, TArray<double>& A2) const
{
...
double UAttentionSpanManager::TrainNetwork(int32 Epochs, double LearningRate)
{
    if (AttentionDataEntries.Num() == 0 || InputSize == 0 || HiddenSize == 0 || OutputSize == 0)
    {
        return 0.0;
    }

    const int32 N = AttentionDataEntries.Num();
    TArray<double> X; X.SetNumUninitialized(N * InputSize);
    TArray<double> Y; Y.SetNumUninitialized(N * OutputSize);

    for (int32 n = 0; n < N; ++n)
    {
        const FAttentionData& Dpt = AttentionDataEntries[n];
        X[n * InputSize + 0] = Dpt.TFocus;
        if (InputSize > 1) X[n * InputSize + 1] = Dpt.D;
        if (InputSize > 2) X[n * InputSize + 2] = Dpt.Lambda;

        Y[n * OutputSize + 0] = Dpt.AttentionSpan; // single output
    }
...
double UAttentionSpanManager::PredictAttentionSpan(double TFocus, double D, double Lambda) const
{
    if (InputSize == 0 || HiddenSize == 0 || OutputSize == 0) return 0.0;

    double In[3] = { TFocus, D, Lambda };
    TArray<double> Z1, A1, Z2, A2;
