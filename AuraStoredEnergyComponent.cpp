#include "AuraStoredEnergyComponent.h"

#include "Components/SceneComponent.h"
#include "Particles/ParticleSystemComponent.h"
#include "NiagaraComponent.h"

UAuraStoredEnergyComponent::UAuraStoredEnergyComponent()
{
    PrimaryComponentTick.bCanEverTick = false;
}

void UAuraStoredEnergyComponent::BeginPlay()
{
    Super::BeginPlay();
    ApplyContentAura();
}

TArray<USceneComponent*> UAuraStoredEnergyComponent::GetAllLayers() const
{
...
void UAuraStoredEnergyComponent::ApplyContentAura()
{
    for (USceneComponent* Layer : GetAllLayers())
    {
        ApplyStoredEnergyToAura(Layer);
    }
}

void UAuraStoredEnergyComponent::ApplyStoredEnergyToAura(USceneComponent* AuraLayer)
{
    if (!AuraLayer)
    {
        return;
...
void UAuraStoredEnergyComponent::ApplyToNiagara(UNiagaraComponent* NiagaraComp) const
{
    if (!NiagaraComp)
    {
        return;
...
void UAuraStoredEnergyComponent::ApplyToCascade(UParticleSystemComponent* ParticleComp) const
{
    if (!ParticleComp)
    {
        return;
