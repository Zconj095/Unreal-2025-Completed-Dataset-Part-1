...
double UJoulesRecognitionManager::Sigmoid(double x)
{
    x = FMath::Clamp(x, -50.0, 50.0);
    return 1.0 / (1.0 + FMath::Exp(-x));
}

double UJoulesRecognitionManager::SigmoidDerivFromActivation(double a)
{
    return a * (1.0 - a);
}

void UJoulesRecognitionManager::RandomizeParams(double Scale)
{
...
void UJoulesRecognitionManager::InitializeNetwork(int32 InInputSize, int32 InHiddenNeurons, int32 InOutputSize)
{
...
void UJoulesRecognitionManager::AddMemoryOperation(const TArray<double>& Features, double Joules)
{
    MemoryOperations.Add(FMemoryOperation(Features, Joules));
}

void UJoulesRecognitionManager::ForwardSingle(const double* InX, TArray<double>& Z1, TArray<double>& A1, TArray<double>& Z2, TArray<double>& A2) const
{
...
double UJoulesRecognitionManager::TrainNetwork(int32 Epochs, double LearningRate)
{
    if (MemoryOperations.Num() == 0 || InputSize == 0 || HiddenSize == 0 || OutputSize == 0)
    {
        return 0.0;
    }

    const int32 N = MemoryOperations.Num();
    TArray<double> X; X.SetNumZeroed(N * InputSize);
    TArray<double> Y; Y.SetNumZeroed(N * OutputSize);
    for (int32 n = 0; n < N; ++n)
    {
        const FMemoryOperation& Op = MemoryOperations[n];
        for (int i = 0; i < InputSize && i < Op.Features.Num(); ++i)
        {
            X[n * InputSize + i] = Op.Features[i];
        }
        Y[n * OutputSize + 0] = Op.JoulesConsumed;
    }
...
double UJoulesRecognitionManager::PredictJoules(const TArray<double>& Features) const
{
...
double UJoulesRecognitionManager::CalculateAverageJoules() const
{
    if (MemoryOperations.Num() == 0) return 0.0;
    double Sum = 0.0;
    for (const FMemoryOperation& Op : MemoryOperations)
    {
        Sum += Op.JoulesConsumed;
    }
    return Sum / static_cast<double>(MemoryOperations.Num());
}
