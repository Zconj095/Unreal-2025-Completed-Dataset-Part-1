...
double UFocusManager::Sigmoid(double x)
{
    x = FMath::Clamp(x, -50.0, 50.0);
    return 1.0 / (1.0 + FMath::Exp(-x));
}

double UFocusManager::SigmoidDerivFromActivation(double a)
{
    return a * (1.0 - a);
}

void UFocusManager::RandomizeParams(double Scale)
{
...
void UFocusManager::InitializeNetwork(int32 InInputSize, int32 InHiddenNeurons, int32 InOutputSize)
{
...
void UFocusManager::AddFocusData(double Rc, double Tc)
{
    if (Tc == 0.0)
    {
        UE_LOG(LogTemp, Warning, TEXT("Tc (Total cognitive load) cannot be zero. Skipping data point."));
        return;
    }
    const double F = CalculateFocus(Rc, Tc);
    Dataset.Add(FFocusData(Rc, Tc, F));
}

double UFocusManager::CalculateFocus(double Rc, double Tc) const
{
    if (Tc == 0.0) return 0.0;
    return Rc / Tc;
}

void UFocusManager::ForwardSingle(const double* InX, TArray<double>& Z1, TArray<double>& A1, TArray<double>& Z2, TArray<double>& A2) const
{
...
double UFocusManager::TrainNetwork(int32 Epochs, double LearningRate)
{
    ...
    for (int32 n = 0; n < N; ++n)
    {
        const FFocusData& D = Dataset[n];
        X[n * InputSize + 0] = D.Rc;
        if (InputSize > 1) X[n * InputSize + 1] = D.Tc;
        Y[n * OutputSize + 0] = D.Focus;
    }
...
double UFocusManager::PredictFocus(double Rc, double Tc) const
{
    if (Tc == 0.0)
    {
        UE_LOG(LogTemp, Warning, TEXT("Tc (Total cognitive load) cannot be zero. Returning 0 for focus."));
        return 0.0;
    }
