...
double UMemoryRetentionRateManager::Sigmoid(double x)
{
    x = FMath::Clamp(x, -50.0, 50.0);
    return 1.0 / (1.0 + FMath::Exp(-x));
}

double UMemoryRetentionRateManager::SigmoidDerivFromActivation(double a)
{
    return a * (1.0 - a);
}

void UMemoryRetentionRateManager::RandomizeParams(double Scale)
{
...
void UMemoryRetentionRateManager::InitializeNetwork(int32 InInputSize, int32 InHiddenNeurons, int32 InOutputSize)
{
...
void UMemoryRetentionRateManager::AddRetentionData(double M0, double Alpha, double Time)
{
    const double Ret = CalculateMemoryRetentionRate(M0, Alpha, Time);
    Entries.Add(FRetentionData(M0, Alpha, Time, Ret));
}

double UMemoryRetentionRateManager::CalculateMemoryRetentionRate(double M0, double Alpha, double Time) const
{
    return M0 * FMath::Exp(-Alpha * Time);
}

void UMemoryRetentionRateManager::ForwardSingle(const double* InX, TArray<double>& Z1, TArray<double>& A1, TArray<double>& Z2, TArray<double>& A2) const
{
...
double UMemoryRetentionRateManager::TrainNetwork(int32 Epochs, double LearningRate)
{
    ...
    for (int32 n = 0; n < N; ++n)
    {
        const FRetentionData& D = Entries[n];
        X[n * InputSize + 0] = D.M0;
        if (InputSize > 1) X[n * InputSize + 1] = D.Alpha;
        if (InputSize > 2) X[n * InputSize + 2] = D.Time;
        Y[n * OutputSize + 0] = D.Retention;
    }
...
double UMemoryRetentionRateManager::PredictMemoryRetentionRate(double M0, double Alpha, double Time) const
{
    ...
    In[0] = M0; if (InputSize > 1) In[1] = Alpha; if (InputSize > 2) In[2] = Time;
