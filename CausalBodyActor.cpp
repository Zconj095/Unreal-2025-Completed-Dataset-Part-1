#include "CausalBodyActor.h"

#include "Components/SceneComponent.h"
#include "NiagaraComponent.h"
#include "Particles/ParticleSystemComponent.h"

ACausalBodyActor::ACausalBodyActor()
{
    Root = CreateDefaultSubobject<USceneComponent>(TEXT("Root"));
    RootComponent = Root;

#if WITH_EDITOR
    SetActorLabel(TEXT("Causal Body"));
#endif
}

void ACausalBodyActor::BeginPlay()
{
    Super::BeginPlay();
    ApplyAuraSettings();
}

void ACausalBodyActor::ApplyAuraSettings()
{
    if (!AuraLayer)
    {
        return;
...
void ACausalBodyActor::ApplyToNiagara(UNiagaraComponent* NiagaraComp) const
{
    if (!NiagaraComp)
    {
        return;
...
void ACausalBodyActor::ApplyToCascade(UParticleSystemComponent* ParticleComp) const
{
    if (!ParticleComp)
    {
        return;
