...
ACelestialBodyActor::ACelestialBodyActor()
{
    Root = CreateDefaultSubobject<USceneComponent>(TEXT("Root"));
    RootComponent = Root;

#if WITH_EDITOR
    SetActorLabel(TEXT("Celestial Body"));
#endif
}

void ACelestialBodyActor::BeginPlay()
{
    Super::BeginPlay();
    ApplyAuraSettings();
}

void ACelestialBodyActor::ApplyAuraSettings()
{
    if (!AuraLayer)
    {
        return;
...
void ACelestialBodyActor::ApplyToNiagara(UNiagaraComponent* NiagaraComp) const
{
    if (!NiagaraComp)
    {
        return;
...
void ACelestialBodyActor::ApplyToCascade(UParticleSystemComponent* ParticleComp) const
{
    if (!ParticleComp)
    {
        return;
