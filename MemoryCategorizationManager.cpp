...
double UMemoryCategorizationManager::Sigmoid(double x)
{
    x = FMath::Clamp(x, -50.0, 50.0);
    return 1.0 / (1.0 + FMath::Exp(-x));
}

double UMemoryCategorizationManager::SigmoidDerivFromActivation(double a)
{
    return a * (1.0 - a);
}

void UMemoryCategorizationManager::RandomizeParams(double Scale)
{
...
void UMemoryCategorizationManager::InitializeNetwork(int32 InInputSize, int32 InHiddenNeurons, int32 InOutputSize)
{
...
void UMemoryCategorizationManager::AddMemoryData(const TArray<double>& Features, int32 Category)
{
    MemoryEntries.Add(FMemoryData(Features, Category));
}

void UMemoryCategorizationManager::ForwardSingle(const double* InX, TArray<double>& Z1, TArray<double>& A1, TArray<double>& Z2, TArray<double>& A2) const
{
...
double UMemoryCategorizationManager::TrainNetwork(int32 Epochs, double LearningRate)
{
    if (MemoryEntries.Num() == 0 || InputSize == 0 || HiddenSize == 0 || OutputSize == 0)
    {
        return 0.0;
    }

    const int32 N = MemoryEntries.Num();
    TArray<double> X; X.SetNumZeroed(N * InputSize);
    TArray<double> Y; Y.SetNumZeroed(N * OutputSize); // one-hot

    for (int32 n = 0; n < N; ++n)
    {
        const FMemoryData& E = MemoryEntries[n];
        for (int i = 0; i < InputSize && i < E.Features.Num(); ++i)
        {
            X[n * InputSize + i] = E.Features[i];
        }
        if (E.Category >= 0 && E.Category < OutputSize)
        {
            Y[n * OutputSize + E.Category] = 1.0;
        }
    }
...
int32 UMemoryCategorizationManager::PredictCategory(const TArray<double>& Features) const
{
...
TMap<int32, double> UMemoryCategorizationManager::GetCategoryStatistics() const
{
    TMap<int32, double> Stats;
    const int32 Total = MemoryEntries.Num();
    if (Total == 0) return Stats;

    for (const FMemoryData& E : MemoryEntries)
    {
        double& Count = Stats.FindOrAdd(E.Category);
        Count += 1.0;
    }
    for (TPair<int32, double>& Pair : Stats)
    {
        Pair.Value = (Pair.Value / static_cast<double>(Total)) * 100.0;
    }
    return Stats;
}
