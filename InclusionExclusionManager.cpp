...
double UInclusionExclusionManager::Sigmoid(double x)
{
    x = FMath::Clamp(x, -50.0, 50.0);
    return 1.0 / (1.0 + FMath::Exp(-x));
}

double UInclusionExclusionManager::SigmoidDerivFromActivation(double a)
{
    return a * (1.0 - a);
}

void UInclusionExclusionManager::RandomizeParams(double Scale)
{
...
void UInclusionExclusionManager::InitializeNetwork(int32 InInputSize, int32 InHiddenNeurons, int32 InOutputSize)
{
...
void UInclusionExclusionManager::ForwardSingle(const double* InX, TArray<double>& Z1, TArray<double>& A1, TArray<double>& Z2, TArray<double>& A2) const
{
...
void UInclusionExclusionManager::TrainNetwork(const TArray<FDoubleArray>& Inputs, const TArray<FDoubleArray>& Outputs, int32 Epochs, double LearningRate)
{
...
FString UInclusionExclusionManager::Predict(const TArray<double>& Input) const
{
    if (InputSize == 0 || HiddenSize == 0 || OutputSize == 0) return TEXT("Exclude");
    ...
    const double v = A2[0];
    return (v > 0.5) ? TEXT("Include") : TEXT("Exclude");
}

TTuple<TArray<double>, TArray<double>> UInclusionExclusionManager::Separate(const TArray<double>& Data, TFunctionRef<bool(double)> DecisionFunction) const
{
    TArray<double> Include;
    TArray<double> Exclude;
    for (double x : Data)
    {
        if (DecisionFunction(x)) Include.Add(x); else Exclude.Add(x);
    }
    return MakeTuple(Include, Exclude);
}

void UInclusionExclusionManager::AddInclusionExclusionData(const TArray<double>& Data, TFunctionRef<bool(double)> DecisionFunction)
{
    auto Pair = Separate(Data, DecisionFunction);
    Entries.Add(FInclusionExclusionData(Data, Pair.Get<0>(), Pair.Get<1>()));
}

void UInclusionExclusionManager::DisplayResults() const
{
    for (const FInclusionExclusionData& E : Entries)
    {
        FString Orig, Inc, Exc;
        for (int i = 0; i < E.OriginalData.Num(); ++i) { if (i) Orig += TEXT(", "); Orig += FString::SanitizeFloat(E.OriginalData[i]); }
        for (int i = 0; i < E.Included.Num(); ++i) { if (i) Inc += TEXT(", "); Inc += FString::SanitizeFloat(E.Included[i]); }
        for (int i = 0; i < E.Excluded.Num(); ++i) { if (i) Exc += TEXT(", "); Exc += FString::SanitizeFloat(E.Excluded[i]); }

        UE_LOG(LogTemp, Log, TEXT("Original Data: %s"), *Orig);
        UE_LOG(LogTemp, Log, TEXT("Included: %s"), *Inc);
        UE_LOG(LogTemp, Log, TEXT("Excluded: %s"), *Exc);
    }
}
