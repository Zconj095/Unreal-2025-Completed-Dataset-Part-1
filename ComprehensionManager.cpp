...
double UComprehensionManager::Sigmoid(double x)
{
    x = FMath::Clamp(x, -50.0, 50.0);
    return 1.0 / (1.0 + FMath::Exp(-x));
}

double UComprehensionManager::SigmoidDerivFromActivation(double a)
{
    return a * (1.0 - a);
}

void UComprehensionManager::RandomizeParams(double Scale)
{
...
void UComprehensionManager::InitializeNetwork(int32 InInputSize, int32 InHiddenNeurons, int32 InOutputSize)
{
...
void UComprehensionManager::AddComprehensionData(double Ip, double Ic, double FocusFactor)
{
    if (Ic == 0.0)
    {
        UE_LOG(LogTemp, Warning, TEXT("Ic (Total cognitive capacity) cannot be zero. Skipping data point."));
        return;
    }
    const double Comp = CalculateComprehension(Ip, Ic, FocusFactor);
    Dataset.Add(FComprehensionData(Ip, Ic, FocusFactor, Comp));
}

double UComprehensionManager::CalculateComprehension(double Ip, double Ic, double FocusFactor) const
{
    // (Ip / Ic) * FocusFactor
    if (Ic == 0.0) return 0.0;
    return (Ip / Ic) * FocusFactor;
}

void UComprehensionManager::ForwardSingle(const double* InX, TArray<double>& Z1, TArray<double>& A1, TArray<double>& Z2, TArray<double>& A2) const
{
...
double UComprehensionManager::TrainNetwork(int32 Epochs, double LearningRate)
{
    ...
    for (int32 n = 0; n < N; ++n)
    {
        const FComprehensionData& D = Dataset[n];
        X[n * InputSize + 0] = D.Ip;
        if (InputSize > 1) X[n * InputSize + 1] = D.Ic;
        if (InputSize > 2) X[n * InputSize + 2] = D.FocusFactor;

        Y[n * OutputSize + 0] = D.Comprehension; // single output
    }
...
double UComprehensionManager::PredictComprehension(double Ip, double Ic, double FocusFactor) const
{
    if (Ic == 0.0)
    {
        UE_LOG(LogTemp, Warning, TEXT("Ic (Total cognitive capacity) cannot be zero. Returning 0 for comprehension."));
        return 0.0;
    }
