...
double ULearningRateAcknowledgmentManager::Sigmoid(double x)
{
    x = FMath::Clamp(x, -50.0, 50.0);
    return 1.0 / (1.0 + FMath::Exp(-x));
}

double ULearningRateAcknowledgmentManager::SigmoidDerivFromActivation(double a)
{
    return a * (1.0 - a);
}

void ULearningRateAcknowledgmentManager::RandomizeParams(double Scale)
{
...
void ULearningRateAcknowledgmentManager::InitializeNetwork(int32 InInputSize, int32 InHiddenNeurons, int32 InOutputSize)
{
...
void ULearningRateAcknowledgmentManager::AddLearningData(double DeltaK, double DeltaT)
{
    const double LR = CalculateLearningRate(DeltaK, DeltaT);
    Dataset.Add(FLearningData(DeltaK, DeltaT, LR));
}

double ULearningRateAcknowledgmentManager::CalculateLearningRate(double DeltaK, double DeltaT) const
{
    if (DeltaT == 0.0)
    {
        UE_LOG(LogTemp, Warning, TEXT("DeltaT (time spent learning) cannot be zero. Returning 0 for learning rate."));
        return 0.0;
    }
    return DeltaK / DeltaT;
}

void ULearningRateAcknowledgmentManager::ForwardSingle(const double* InX, TArray<double>& Z1, TArray<double>& A1, TArray<double>& Z2, TArray<double>& A2) const
{
...
double ULearningRateAcknowledgmentManager::TrainNetwork(int32 Epochs, double LearningRate)
{
    ...
    for (int32 n = 0; n < N; ++n)
    {
        const FLearningData& D = Dataset[n];
        X[n * InputSize + 0] = D.DeltaK;
        if (InputSize > 1) X[n * InputSize + 1] = D.DeltaT;
        Y[n * OutputSize + 0] = D.LearningRate;
    }
...
double ULearningRateAcknowledgmentManager::PredictLearningRate(double DeltaK, double DeltaT) const
{
    if (DeltaT == 0.0)
    {
        UE_LOG(LogTemp, Warning, TEXT("DeltaT (time spent learning) cannot be zero. Returning 0 for learning rate."));
