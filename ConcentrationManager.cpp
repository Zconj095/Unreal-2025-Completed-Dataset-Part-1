...
double UConcentrationManager::Sigmoid(double x)
{
    x = FMath::Clamp(x, -50.0, 50.0);
    return 1.0 / (1.0 + FMath::Exp(-x));
}

double UConcentrationManager::SigmoidDerivFromActivation(double a)
{
    return a * (1.0 - a);
}

void UConcentrationManager::RandomizeParams(double Scale)
{
...
void UConcentrationManager::InitializeNetwork(int32 InInputSize, int32 InHiddenNeurons, int32 InOutputSize)
{
...
void UConcentrationManager::AddConcentrationData(double Focus, double Distraction, double Susceptibility)
{
    const double Con = CalculateConcentration(Focus, Distraction, Susceptibility);
    Dataset.Add(FConcentrationData(Focus, Distraction, Susceptibility, Con));
}

double UConcentrationManager::CalculateConcentration(double Focus, double Distraction, double Susceptibility) const
{
    // Focus / (1 + Distraction * Susceptibility)
    return Focus / (1.0 + Distraction * Susceptibility);
}

void UConcentrationManager::ForwardSingle(const double* InX, TArray<double>& Z1, TArray<double>& A1, TArray<double>& Z2, TArray<double>& A2) const
{
...
double UConcentrationManager::TrainNetwork(int32 Epochs, double LearningRate)
{
    ...
    for (int32 n = 0; n < N; ++n)
    {
        const FConcentrationData& D = Dataset[n];
        X[n * InputSize + 0] = D.Focus;
        if (InputSize > 1) X[n * InputSize + 1] = D.Distraction;
        if (InputSize > 2) X[n * InputSize + 2] = D.Susceptibility;

        Y[n * OutputSize + 0] = D.Concentration; // single output
    }
...
double UConcentrationManager::PredictConcentration(double Focus, double Distraction, double Susceptibility) const
{
    ...
    double In[3] = { Focus, Distraction, Susceptibility };
    TArray<double> Z1, A1, Z2, A2;
