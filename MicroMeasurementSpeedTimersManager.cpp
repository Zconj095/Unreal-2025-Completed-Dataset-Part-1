...
double UMicroMeasurementSpeedTimersManager::Sigmoid(double x)
{
    x = FMath::Clamp(x, -50.0, 50.0);
    return 1.0 / (1.0 + FMath::Exp(-x));
}

double UMicroMeasurementSpeedTimersManager::SigmoidDerivFromActivation(double a)
{
    return a * (1.0 - a);
}

void UMicroMeasurementSpeedTimersManager::RandomizeParams(double Scale)
{
...
void UMicroMeasurementSpeedTimersManager::InitializeNetwork(int32 InInputSize, int32 InHiddenNeurons, int32 InOutputSize)
{
    InputSize = InInputSize;
    HiddenSize = InHiddenNeurons;
    OutputSize = InOutputSize; // normally 1
    RandomizeParams(0.1);
}

void UMicroMeasurementSpeedTimersManager::AddTimerData(const TArray<double>& Features, double MicroSeconds)
{
    TimerEntries.Add(FMicroTimerData(Features, MicroSeconds));
}

void UMicroMeasurementSpeedTimersManager::ForwardSingle(const double* InX, TArray<double>& Z1, TArray<double>& A1, TArray<double>& Z2, TArray<double>& A2) const
{
...
double UMicroMeasurementSpeedTimersManager::TrainNetwork(int32 Epochs, double LearningRate)
{
    if (TimerEntries.Num() == 0 || InputSize == 0 || HiddenSize == 0 || OutputSize == 0)
    {
        return 0.0;
    }

    const int32 N = TimerEntries.Num();
    TArray<double> X; X.SetNumUninitialized(N * InputSize);
    TArray<double> Y; Y.SetNumUninitialized(N * OutputSize);

    for (int32 n = 0; n < N; ++n)
    {
        const FMicroTimerData& D = TimerEntries[n];
        for (int i = 0; i < InputSize; ++i)
        {
            X[n * InputSize + i] = (i < D.Features.Num()) ? D.Features[i] : 0.0;
        }
        Y[n * OutputSize + 0] = D.MicroSeconds;
    }
...
double UMicroMeasurementSpeedTimersManager::PredictMicroSeconds(const TArray<double>& Features) const
{
    ...
    // Prepare input vector sized to InputSize
...
double UMicroMeasurementSpeedTimersManager::CalculateAverageTime() const
{
    if (TimerEntries.Num() == 0) return 0.0;
    double Sum = 0.0;
    for (const FMicroTimerData& E : TimerEntries)
    {
        Sum += E.MicroSeconds;
    }
    return Sum / static_cast<double>(TimerEntries.Num());
}
