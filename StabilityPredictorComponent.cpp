#include "StabilityPredictorComponent.h"
...
UStabilityPredictorComponent::UStabilityPredictorComponent()
{
    PrimaryComponentTick.bCanEverTick = false;
}

void UStabilityPredictorComponent::BeginPlay()
{
    Super::BeginPlay();
    InitializeNetwork();
}

void UStabilityPredictorComponent::InitializeNetwork()
{
    AllocateWeights();
}

void UStabilityPredictorComponent::AllocateWeights()
{
...
double UStabilityPredictorComponent::Sigmoid(double Value) const
{
    return 1.0 / (1.0 + FMath::Exp(-Value));
}

void UStabilityPredictorComponent::Forward(const TArray<double>& Inputs, TArray<double>& Outputs) const
{
    HiddenLayerValues.Init(0.0, HiddenSize);
...
TArray<double> UStabilityPredictorComponent::Predict(const TArray<double>& Inputs) const
{
    TArray<double> Outputs;
    if (Inputs.Num() != InputSize)
    {
        UE_LOG(LogTemp, Warning, TEXT("Stability predictor input size mismatch."));
        return Outputs;
    }
...
void UStabilityPredictorComponent::Train(const TArray<FDoubleArrayWrapper>& Inputs, const TArray<FDoubleArrayWrapper>& Targets, int32 Epochs, double LearningRate)
{
    if (Inputs.Num() != Targets.Num() || Inputs.Num() == 0)
    {
        UE_LOG(LogTemp, Warning, TEXT("Invalid training data for stability predictor."));
        return;
    }
