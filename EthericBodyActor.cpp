#include "EthericBodyActor.h"

#include "Components/SceneComponent.h"
#include "NiagaraComponent.h"
#include "Particles/ParticleSystemComponent.h"

AEthericBodyActor::AEthericBodyActor()
{
    Root = CreateDefaultSubobject<USceneComponent>(TEXT("Root"));
    RootComponent = Root;

#if WITH_EDITOR
    SetActorLabel(TEXT("Etheric Body"));
#endif
}

void AEthericBodyActor::BeginPlay()
{
    Super::BeginPlay();
    ApplyAuraSettings();
}

void AEthericBodyActor::ApplyAuraSettings()
{
    if (!AuraLayer)
    {
        return;
...
void AEthericBodyActor::ApplyToNiagara(UNiagaraComponent* NiagaraComp) const
{
    if (!NiagaraComp)
    {
        return;
...
void AEthericBodyActor::ApplyToCascade(UParticleSystemComponent* ParticleComp) const
{
    if (!ParticleComp)
    {
        return;
