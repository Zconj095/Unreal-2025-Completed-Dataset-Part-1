#include "AuraForcedEnergyComponent.h"

#include "Components/SceneComponent.h"
#include "Particles/ParticleSystemComponent.h"
#include "NiagaraComponent.h"

UAuraForcedEnergyComponent::UAuraForcedEnergyComponent()
{
    PrimaryComponentTick.bCanEverTick = false;
}

void UAuraForcedEnergyComponent::BeginPlay()
{
    Super::BeginPlay();
    ApplyContentAura();
}

TArray<USceneComponent*> UAuraForcedEnergyComponent::GetAllLayers() const
{
...
void UAuraForcedEnergyComponent::ApplyContentAura()
{
    for (USceneComponent* Layer : GetAllLayers())
    {
        ApplyForcedEnergyToAura(Layer);
    }
}

void UAuraForcedEnergyComponent::ApplyForcedEnergyToAura(USceneComponent* AuraLayer)
{
    if (!AuraLayer)
    {
        return;
...
void UAuraForcedEnergyComponent::ApplyToNiagara(UNiagaraComponent* NiagaraComp) const
{
    if (!NiagaraComp)
    {
        return;
...
void UAuraForcedEnergyComponent::ApplyToCascade(UParticleSystemComponent* ParticleComp) const
{
    if (!ParticleComp)
    {
        return;
