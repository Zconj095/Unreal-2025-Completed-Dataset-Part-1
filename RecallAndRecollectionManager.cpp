...
double URecallAndRecollectionManager::Sigmoid(double x)
{
    x = FMath::Clamp(x, -50.0, 50.0);
    return 1.0 / (1.0 + FMath::Exp(-x));
}

double URecallAndRecollectionManager::SigmoidDerivFromActivation(double a)
{
    return a * (1.0 - a);
}

void URecallAndRecollectionManager::RandomizeParams(double Scale)
{
...
void URecallAndRecollectionManager::InitializeNetwork(int32 InInputSize, int32 InHiddenNeurons, int32 InOutputSize)
{
...
void URecallAndRecollectionManager::AddRecallData(double M, double Beta, double Time, double ContextFactor)
{
    const double R = CalculateRecall(M, Beta, Time);
    const double Re = CalculateRecollection(R, ContextFactor);
    RecallEntries.Add(FRecallData(M, Beta, Time, ContextFactor, R, Re));
}

double URecallAndRecollectionManager::CalculateRecall(double M, double Beta, double Time) const
{
    return M * FMath::Exp(-Beta * Time);
}

double URecallAndRecollectionManager::CalculateRecollection(double R, double ContextFactor) const
{
    return R * ContextFactor;
}

void URecallAndRecollectionManager::ForwardSingle(const double* InX, TArray<double>& Z1, TArray<double>& A1, TArray<double>& Z2, TArray<double>& A2) const
{
...
double URecallAndRecollectionManager::TrainNetwork(int32 Epochs, double LearningRate)
{
    if (RecallEntries.Num() == 0 || InputSize == 0 || HiddenSize == 0 || OutputSize == 0)
    {
        return 0.0;
    }

    const int32 N = RecallEntries.Num();
    TArray<double> X; X.SetNumUninitialized(N * InputSize);
    TArray<double> Y; Y.SetNumUninitialized(N * OutputSize);

    for (int32 n = 0; n < N; ++n)
    {
        const FRecallData& D = RecallEntries[n];
        // Input: [M, Beta, Time, ContextFactor]
        X[n * InputSize + 0] = D.M;
        if (InputSize > 1) X[n * InputSize + 1] = D.Beta;
        if (InputSize > 2) X[n * InputSize + 2] = D.Time;
        if (InputSize > 3) X[n * InputSize + 3] = D.ContextFactor;

        // Output: [R, Re]
        Y[n * OutputSize + 0] = D.R;
        if (OutputSize > 1) Y[n * OutputSize + 1] = D.Re;
    }
...
void URecallAndRecollectionManager::PredictRecallAndRecollection(double M, double Beta, double Time, double ContextFactor, double& OutRecall, double& OutRecollection) const
{
    ...
    In[0] = M;
    if (InputSize > 1) In[1] = Beta;
    if (InputSize > 2) In[2] = Time;
    if (InputSize > 3) In[3] = ContextFactor;

    TArray<double> Z1, A1, Z2, A2;
    ForwardSingle(In.GetData(), Z1, A1, Z2, A2);

    OutRecall = (OutputSize >= 1) ? A2[0] : 0.0;
    OutRecollection = (OutputSize >= 2) ? A2[1] : 0.0;
}
